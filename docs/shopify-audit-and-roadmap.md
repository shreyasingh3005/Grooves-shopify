...
Prepared 21 September 2026. Baseline: local commit `7be9b1a`, Savor 4.1.5, exported theme in `D:/Grooves shopify`. This is an implementation specification, not a claim that the storefront has been repaired or certified for launch.
...
The most urgent confirmed findings are fabricated product ratings, hero CTAs that cannot render because their links are blank, unsafe variant selection in custom add-to-cart forms, inconsistent commercial promises, and customer-visible placeholder content. These are more consequential than cosmetic polish.
...
|---|---|---|---|---|
| A01 | P0 · trust / reviews | `sections/custom-product-slider.liquid:206` derives ratings from product IDs; line 219 substitutes 4.8 when reviews are absent. `templates/index.json` enables `use_random_rating` in both custom sliders; `templates/product.json` enables it again. | Remove random-rating settings and code, including fallback stars. Render genuine rating and count only when source data exists. Remove fabricated values from any feeds/schema if found. | An unrated product has no numerical rating; rated cards match the approved review source. 3–5 h. |
...
| A03 | P0 · cart correctness | `sections/custom-product-slider.liquid:243–246` posts `product.variants.first.id` while enabling ADD from `product.available`. A sold-out first variant with another available variant enables an invalid purchase attempt; multi-option items bypass customer choice. | Reuse native product cards/quick add. Single default-variant products can add directly; products with choices must open the native selector. Preserve selling plans, quantity rules, line properties, and inventory checks. Use locale-aware routes. | Test first variant sold out, all sold out, multiple options, required selling plan, and rapid repeat clicks; correct line appears once. 4–8 h. |
...
| A05 | P0 · incomplete content | `templates/product.json → shoppable_videos_AbiFWj` and `templates/cart.json → shoppable_videos_X6Fg7L` each contain three unassigned video/product blocks. `sections/shoppable-videos.liquid:158–185` emits `#` links and “Upload a Video” / “Assign a Product” placeholders. | Hide incomplete blocks outside design mode; remove empty cart/PDP video sections for MVP. Require both valid product and media for shoppable content. Audit all export handles in admin. | No public placeholder or dead shoppable card on any principal template. 2–4 h. |
...
| A14 | P1 · SEO | Canonical, title, conditional description, OG/Twitter, product and article schema already exist. No BreadcrumbList reference was found in reviewed theme files. Export does not include admin SEO fields. | Populate real admin metadata; add visible breadcrumbs and matching JSON-LD once; validate existing schema before extending it. Confirm apps do not duplicate entities. | Every indexable URL has unique metadata, one correct canonical and eligible accurate schema. 1–2 days plus catalog writing. |
| A15 | P1 · social previews | `snippets/meta-tags.liquid:60` sets `og:image` to HTTP, alongside HTTPS secure_url. `og:site_name` lacks escaping; price uses localized money text. | Normalize OG media to absolute HTTPS; escape strings; verify machine-readable price format without grouping symbols; set a share-image fallback. Test Markets currency. | Actual shared URL produces correct title, image and price without malformed attributes. 2–3 h. |
